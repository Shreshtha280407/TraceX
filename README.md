...
Explicitly **not** implemented anywhere in this repository yet: a continuous worker loop for `structured_processing`/`communication_processing` (`media_processing`/`graph` gained one in the Phase 2 closeout, `--once` remains available for all four), real OCR/ASR for scanned documents/audio transcription (`media_processing`'s own real, local Tesseract OCR is unrelated — it reads video/image frames, not scanned PDF pages or spoken audio), automatic entity resolution or identity merging, face recognition or person re-identification, cross-modal correlation, a human-review workflow, Merkle checkpointing or signatures, case CRUD, worker login/logout or OAuth/OIDC/SAML/mTLS, MFA/SSO, or any frontend. See `CLAUDE.md`, `docs/architecture/phase-1-decisions.md`, `docs/architecture/phase-2-decisions.md`, `docs/architecture/worker-job-lifecycle.md`, `docs/architecture/worker-identity-and-security.md`, `docs/architecture/graph-projection.md`, `docs/architecture/media-processing-worker.md`, and `docs/qa/known-limitations.md` for the full non-goal list.
