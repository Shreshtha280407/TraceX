...
- [x] Seven processors reused unchanged from Phase 1 (`SUPPORTED_PROCESSORS`): `audio_metadata_v1`, `transcript_import_v1`, `diarization_import_v1`, `whatsapp_export_v1`, `telegram_export_v1`, `instagram_export_v1`, `generic_social_json_v1` — exact names/versions/parsing logic untouched. Only two (`audio_metadata_v1` via `SourceType.AUDIO`, `generic_social_json_v1` via `SourceType.CHAT`) are reachable through the live upload path today — the other five's missing routes are reported (not implemented) for team review, mirroring Phase 2.2's original `generic_tabular_v1`/`generic_json_v1` precedent.
...
- [ ] The routing gap: `transcript_import_v1`/`diarization_import_v1`/`whatsapp_export_v1`/`telegram_export_v1`/`instagram_export_v1` are fully implemented and tested but unreachable through a real evidence upload — needs `evidence_lifecycle/routing.py`'s owner to choose between a server-validated `parser_profile` hint or new per-platform `SourceType`s (see `docs/architecture/communication-processing-worker.md`/`phase-2-decisions.md`).
...
- Worker orchestration invoking `media_processing.worker.process_job` and `graph.projection` from a real ingestion pipeline; a MinIO-backed `SourceResolver` (`structured_processing.worker.process_job`/`communication_processing.worker.process_job` are both now callable via their own one-shot `--once` CLIs).
