...
**Resolved (Phase 2.3)**: `evidence_lifecycle/routing.py` originally only ever created `fir_report_text_v1`/`cdr_generic_v1`/`financial_transaction_generic_v1` jobs from a real evidence upload (`SourceType.DOCUMENT`/`CDR`/`FINANCIAL`) — `generic_tabular_v1`/`generic_json_v1` were supported by this worker from the start, but were unreachable through the live upload path. Phase 2.3 added two new, additive `SourceType` values (`structured_tabular` → `text/csv`/XLSX, `structured_json` → `application/json`) that route to exactly these two profiles, so a real upload can now reach them — see `docs/architecture/evidence-lifecycle.md`'s routing table and `docs/architecture/phase-2-decisions.md`'s "Phase 2.3 Decisions". No change was needed in this worker itself: `generic_tabular_v1`/`generic_json_v1` were already in `SUPPORTED_PROCESSORS`.
