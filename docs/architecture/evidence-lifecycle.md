...
- `EvidenceRecordV1.processing_status` starts directly at `queued`, never `uploaded`, because evidence and its job are created in one atomic transaction — there is no separately observable intermediate state in this phase. **Resolved** (frontend Phase 8 Part 2, Phase 4): a real `claim_job` now also transitions it to `processing` in the same transaction as the job's own `running` transition, and a real terminal `submit_result` transitions it to `processed` (`WorkerStatus.SUCCEEDED`) or `failed` (`FAILED`/`DEFERRED`/`CANCELLED` — `EvidenceProcessingStatus` has no separate deferred/cancelled state) in the same transaction as the job's own terminal transition; `reprocess_evidence`'s `create_job` resets it back to `queued`. See `tests/integration/evidence_lifecycle/test_evidence_lifecycle_live.py::test_processing_status_transitions_through_a_real_claim_and_result`.
