...
| Type | Purpose |
|---|---|
| `ObservationBatchSubmissionV1` | One partial batch: `job_id`, `case_id`, `evidence_id`, `batch_id`, `batch_sequence`, `idempotency_key`, `observations` (list of `ObservationV1`), `transformations` (list of `TransformationProvenanceV1`), `progress` (`ObservationBatchProgressV1` \| null), `submitted_at`, `is_final_batch` |
| `TransformationProvenanceV1` | One durable processing-step record: `transformation_id`, `case_id`, `evidence_id`, `job_id`, `batch_id`, `ordinal`, `step_name`, `step_version`, `config_hash`, `model_version`, `input_locator` (`SourceLocator` \| null), `output_observation_ids`, `derived_artifact_refs` (list of `DerivedArtifact`), `status` (`TransformationStatus`), `started_at`, `completed_at`, `safe_metadata` |
| `ObservationBatchProgressV1` | One progress report: `stage`, `units_total` (int \| null), `units_completed`, `observations_emitted`, `batch_sequence`, `message_code` (str \| null), `occurred_at` |
| `ObservationBatchReceiptV1` | Safe acknowledgement: `job_id`, `batch_id`, `status` (`BatchAcceptanceStatus`: `accepted` \| `replayed`), `accepted_observation_count`, `progress`, `request_id` |

`TransformationStatus`: `succeeded`, `partial`, `failed`. `batch_id`/`idempotency_key` are opaque tokens matching `^[A-Za-z0-9_.-]{1,128}$`. `safe_metadata` rejects secret-shaped keys (`password`, `secret`, `token`, `credential`, `apikey`, `stderr`, `object_uri`, `dsn`, etc., case-insensitively, at any nesting depth) and string values over 500 characters — never raw document/media content, credentials, or stack traces. An `ObservationBatchSubmissionV1` with empty `observations` *and* empty `transformations` *and* no `progress` is rejected as meaningless. See `docs/architecture/phase-3-decisions.md` for the full design reasoning and `docs/architecture/evidence-lifecycle.md`'s "Observation-batch ingestion" section for the persistence/replay/conflict semantics.

As of Phase 2, `app/modules/evidence_lifecycle/` is a real, durable producer of `WorkerJobV1` (persisted in `worker_jobs`, published to Redis) — see `docs/architecture/evidence-lifecycle.md`. As of Phase 2.1, the same module also validates and durably persists a worker-*submitted* `WorkerResultV1` (and the `ObservationV1`s inside it) via `POST /api/v1/internal/worker-jobs/{job_id}/result` — see `docs/architecture/worker-job-lifecycle.md`. As of Jasraj's Phase 2 structured-processing worker, `app/modules/structured_processing/worker.py`'s one-shot CLI (`uv run python -m app.modules.structured_processing.worker --once`) is the first real *consumer* of `WorkerJobV1`/producer of a submitted `WorkerResultV1` in this repository — see `docs/architecture/structured-processing-worker.md`, including its documented gap in retrieving evidence bytes/metadata for a claimed job. As of Sarthak's Phase 2 communication-processing worker, `app/modules/communication_processing/worker.py`'s equivalent one-shot CLI (`uv run python -m app.modules.communication_processing.worker --once`) is a second, independent real consumer/producer — see `docs/architecture/communication-processing-worker.md`, including its documented routing gap for five of its seven processor profiles. As of Gaurav's Phase 2 completion, `app/modules/media_processing/worker.py`'s equivalent one-shot CLI (`uv run python -m app.modules.media_processing.worker --once`) is a third, independent real consumer/producer for image/video evidence — see `docs/architecture/media-processing-worker.md`, including a second routing-adjacent gap it found (`video/x-matroska` accepted by routing but unrecognized by this module's own `MediaKind`, fixed additively). None of the three workers changes a `V1` contract field, type, or validation rule; `observation_type` remains free-form for exactly this reason (each modality-specific worker introduces its own values — `audio_metadata`, `transcript_segment`, `diarization_speaker_turn`, `chat_message`, `media_metadata`, `object_detection`, `text_region_detection`, `anonymous_track_segment`, `ocr_text_mention`, alongside `structured_processing`'s `document_text_mention`-family/`cdr_call_record`/`financial_transaction_record`/`tabular_record`/`json_scalar_value` — without needing a schema change). As of Nipun's Phase 2 closeout, `media_processing.worker` runs a real, local, checksum-verified/runtime-checked detector/OCR/tracker behind the same unchanged `ObservationV1` shape (`object_detection`/`text_region_detection`/`ocr_text_mention`/`anonymous_track_segment` observations are now genuinely populated by a real model in the live path, not fake-only), and both this worker and the graph projector gained a continuous `--loop` mode plus a new internal `POST /api/v1/internal/worker-jobs/{job_id}/renew` lease-renewal endpoint — none of which touches `app/contracts/` either; `renew`'s response (`RenewLeaseResponse`) is an `evidence_lifecycle`-owned API shape, not a shared contract. `WorkerProgressV1` remains contract-only — the newer `ObservationBatchProgressV1` (Phase 3) is a distinct, richer type serving a different purpose (durable, replayable micro-batch progress history, not an ephemeral status ping) and does not replace it. As of Phase 3 (Nipun), `app/modules/evidence_lifecycle/` also validates and durably persists a worker-submitted `ObservationBatchSubmissionV1` via `POST /api/v1/internal/worker-jobs/{job_id}/observations`, atomically alongside its `TransformationProvenanceV1` records and one `ObservationBatchProgressV1` event — see `docs/architecture/phase-3-decisions.md` and `docs/architecture/evidence-lifecycle.md`. As of Aditya's Phase 3 completion (securing the worker submission boundary — retry limits, lease-renewal ceiling, and the audit events around all of it), **no `app/contracts/` file changed at all**: `max_attempts` lives only on the internal `WorkerJobRecord` (`app/modules/evidence_lifecycle/models.py`), never on the frozen `WorkerJobV1`, and the synthetic terminal result a retry-exhausted job receives is a genuine, ordinarily-shaped `WorkerResultV1` (`error.code="retry_exhausted"`) — not a new contract variant. See `docs/architecture/worker-job-lifecycle.md` and `docs/architecture/worker-identity-and-security.md`. As of Jasraj's Phase 3 real document/OCR/NER/relation extraction and CDR/finance vectorized batch processing, **no `app/contracts/` file changed at all either**: `structured_processing.worker` now produces `ObservationBatchSubmissionV1`s (real OCR/regex/NER/relation mentions for documents; real chunked CDR/finance records) through the identical, unmodified contract Nipun's Phase 3 work already froze, and consumes `WorkerJobV1`/emits `WorkerResultV1` exactly as every worker before it — `observation_type` remains free-form for exactly the same pre-existing reason, now also covering `document_page_ocr_text`, `ner_entity_mention`, `person_contact_association`, `dated_communication_reference`, `transaction_claim`, and `incident_event_mention` alongside the pre-existing `structured_processing` observation types. See `docs/architecture/document-structured-processing.md`.
