...
- **Resolved (Gap-Closure follow-up): `project_entity` is now wired.** `app/modules/graph/entity_graph_sync.py` composes it (plus `project_entity_resolution_candidate`) with WP-2's real Postgres entity/candidate records -- see `docs/qa/known-limitations.md`'s "WP-2 has no Neo4j footprint" entry, now closed. Callers: `intelligence_worker.resolve_entities_once` (`--resolve-entities`, batch) and `entity_api.submit_entity_resolution_review` (best-effort, per-decision). This is a separate outbox from `graph_projection_jobs` above -- entity/candidate projection is synchronous within these two call sites, not queued.
- **`project_event` remains unwired.** Unlike `project_entity`, this is not a missing composition step: no service anywhere in this codebase constructs a real `EventV1` yet (see `docs/architecture/graph-taxonomy-v1.md`). There is no real data to project -- wiring it would mean inventing event semantics no phase has designed, not connecting existing pieces.
