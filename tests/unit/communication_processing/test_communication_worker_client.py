...
names, status-code handling); `tests/integration/communication_processing/
...
_WORKER_TOKEN = "unit-test-worker-token"  # noqa: S105
...
def test_claim_returns_job_when_available() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        assert body == {"processor_name": "audio_metadata_v1", "processor_version": "1.0.0"}
    ...
    result = _client(handler).claim(processor_name="audio_metadata_v1", processor_version="1.0.0")
...
def test_claim_raises_auth_error_on_401() -> None:
    ...
    with pytest.raises(WorkerAuthenticationError):
        _client(handler).claim(processor_name="audio_metadata_v1", processor_version="1.0.0")


def test_claim_raises_auth_error_on_403_scope_denied() -> None:
    """A processor outside this worker's `allowed_processor_names` scope."""
...
def test_fetch_input_success() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            200,
            content=b"synthetic wav bytes",
            headers={
                "content-type": "audio/wav",
                "content-disposition": "attachment; filename=\"a.wav\"; filename*=UTF-8''a.wav",
                "x-tracex-evidence-sha256": sha256,
            },
        )

    resolved = _client(handler).fetch_input(job_id, claim_token="tok-123")
    assert resolved.data == b"synthetic wav bytes"
    assert resolved.content_type == "audio/wav"
    assert resolved.original_filename == "a.wav"
...
def test_fetch_input_rejects_oversized_response() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:  # noqa: ARG001
        return httpx.Response(200, content=oversized, headers={"content-type": "audio/wav"})
