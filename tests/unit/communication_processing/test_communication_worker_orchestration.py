...
from app.modules.communication_processing.client import ClaimResult, SubmitResultAck
...
def test_run_once_happy_path_transcript_import_json_interchange() -> None:
    ...
    assert submitted_result.status is WorkerStatus.SUCCEEDED  # type: ignore[attr-defined]
    assert submitted_result.observations[0].observation_type == "transcript_segment"  # type: ignore[attr-defined]
...
def test_run_once_happy_path_diarization_import_json_interchange() -> None:
    ...
    assert outcome.result_status == "succeeded"
    submitted_result = client.submit_calls[0][2]
    assert submitted_result.observations[0].observation_type == "diarization_speaker_turn"  # type: ignore[attr-defined]
...
def test_run_once_happy_path_social_export() -> None:
    ...
    assert outcome.result_status == "succeeded"
    submitted_result = client.submit_calls[0][2]
    assert submitted_result.observations[0].observation_type == "chat_message"  # type: ignore[attr-defined]
