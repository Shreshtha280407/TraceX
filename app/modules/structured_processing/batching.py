...
Every worker in this module (document, CDR, finance) submits its
observations the same way: a bounded batch of `ObservationV1`s, optional
`TransformationProvenanceV1` records for the steps that produced them, and
at most one `ObservationBatchProgressV1` update — see
`app/contracts/observation_batch.py` for the frozen contract this builds
against. This module owns only the *shape*, never the persistence/replay
semantics, which remain entirely Nipun's (`EvidenceLifecycleService.
